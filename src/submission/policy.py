...
class BasePolicy(ABC):
    ...
    def act(self, observations):
        ...
        ### START CODE HERE ###
        distribution_over_actions = self.action_distribution(observations)
        sampled_actions = distribution_over_actions.sample().cpu().numpy()
        ### END CODE HERE ###
...
class CategoricalPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        """
        ### START CODE HERE ###
        logits = self.network(observations)  # unnormalized probabilities for each action
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        distribution = ptd.multivariate_normal.MultivariateNormal(mean, scale_tril=scale_tril)
        ### END CODE HERE ###
        return distribution
