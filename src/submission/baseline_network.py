...
class BaselineNetwork(nn.Module):
    ...
    def __init__(self, env, config):
        ...
        self.network = build_mlp(input_size=input_size, output_size=output_size, n_layers=n_layers, size=size).to(
            self.device)
    ...
    def calculate_advantage(self, returns, observations):
        ...
        ### START CODE HERE ###
        baseline_values = self.forward(observations).detach().cpu().numpy()
    ...
    def update_baseline(self, returns, observations):
        ...
        loss = loss_func(baseline_values, returns)
